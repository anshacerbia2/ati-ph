...
type PolicyResponse = { policies: Policy[]; pagination: Pagination; error?: string };
        ...
        <div className="notification-policy-card__status">
          {current ? (
            <>
              ...
              <span className={current.scheduleReady ? "ati-badge ati-badge--success" : "ati-badge ati-badge--warning"}>
      ...
      {expanded ? (
        <div className="notification-policy-card__body">
          <div className="notification-policy-facts">
            ...
            <PolicyFact label="Local send" value={current?.sendTimeLocal && current.timezone ? `${current.sendTimeLocal} · ${current.timezone}` : "Not configured"} />
            <PolicyFact label="Approval" value={current?.approvalMode ?? "UNCONFIRMED"} />
          </div>

          {current && current.scheduleIssues.length > 0 ? (
            <div className="notification-policy-issues">
              <strong>Configuration still required</strong>
              <div>{current.scheduleIssues.map((issue) => <span key={issue}>{humanize(issue)}</span>)}</div>
            </div>
          ) : null}
