...
describe("holiday revision control", () => {
  ...
  it("allows a committed target because publication now enforces the live delivery boundary", () => {
    expect(
      validateRevisionTargetStates(
        [
          {
  ...
  it("blocks correction while a delivery is processing", () => {
    expect(
      revisionDeliveryBoundary([
        "SENT",
        "PROCESSING",
  ...
  it("allows correction after sent evidence while identifying cancellable jobs", () => {
    expect(
      revisionDeliveryBoundary([
        "SENT",
        "PLANNED",
