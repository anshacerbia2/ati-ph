...
import { listNotificationPolicies } from "@/notifications/policy";

export const runtime = "nodejs";

export async function GET(request: Request): Promise<Response> {
  const access = await authorizeRoute(PERMISSIONS.NOTIFICATION_POLICY_READ);
  ...
  try {
    ...
    return Response.json(await listNotificationPolicies(query));
  } catch (error) {
    return notificationErrorResponse(error, "policy list");
  }
}
